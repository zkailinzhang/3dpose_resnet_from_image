...
def read_data():
    ...
    demo_dir_path = os.path.join(cfg.DATA_DIR, 'demo_driller')
    rgb = Image.open(os.path.join(demo_dir_path, 'driller.jpg'))
    #掩码是一个真值，
    mask = np.array(Image.open(os.path.join(demo_dir_path, 'driller_mask.png'))).astype(np.int32)[..., 0]
    # 全1 ，，没有概率  权重
    mask[mask != 0] = 1
